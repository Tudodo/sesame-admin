...
/// Keys are namespaced with a `cache:` prefix so cache administration and
/// cache sync can delete only cache keys instead of flushing the shared Redis
/// database (sessions, queues, and shared runtime state live in the same DB).
...
impl CacheDriver for Redis {
    ...
    async fn contains_key(&self, key: &str) -> CacheResult<bool> {
        ...
        Ok(connection.exists(cache_key(key)).await?)
    }
    ...
    async fn get(&self, key: &str) -> CacheResult<Option<String>> {
        let mut conn = self.pool.get().await?;
        let result: Option<String> = conn.get(cache_key(key)).await?;
        Ok(result)
    }
    ...
    async fn insert(&self, key: &str, value: &str) -> CacheResult<()> {
        let mut conn = self.pool.get().await?;
        conn.set::<_, _, ()>(cache_key(key), value).await?;
        Ok(())
    }
    ...
    async fn insert_with_expiry(
        ...
        conn.set_ex::<_, _, ()>(cache_key(key), value, duration.as_secs())
            .await?;
        Ok(())
    }
    ...
    async fn clear(&self) -> CacheResult<()> {
        ...
        loop {
            let (next_cursor, batch): (u64, Vec<String>) = cmd("SCAN")
                ...
                .await?;
            if !batch.is_empty() {
                let n: u64 = cmd("DEL").arg(&batch).query_async(&mut *conn).await?;
                deleted += n;
            }
            cursor = next_cursor;
