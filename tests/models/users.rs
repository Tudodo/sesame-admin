...
async fn can_create_with_password() {
    ...
    let res = Model::create_with_password(&boot.app_context.db, &params, None).await;
...
async fn can_find_by_email() {
    ...
    let existing_user = Model::find_by_email_in_tenant(
        &boot.app_context.db,
        "user1@example.com",
        DEFAULT_TENANT_CODE,
    )
    .await;
    let non_existing_user_results = Model::find_by_email_in_tenant(
        &boot.app_context.db,
        "un@existing-email.com",
        DEFAULT_TENANT_CODE,
    )
    .await;
...
#[tokio::test]
#[serial]
async fn find_by_pid_is_scoped_to_tenant() {
    let boot = boot_test::<App>()
        .await
        .expect("Failed to boot test application");
    seed::<App>(&boot.app_context)
    ...
    let pid = uuid::Uuid::parse_str("11111111-1111-1111-1111-111111111111").unwrap();
    assert!(
        Model::find_by_pid_in_tenant(&boot.app_context.db, pid, DEFAULT_TENANT_CODE)
            .await
            .is_ok(),
        "User should be found in its own tenant"
    );
    assert!(
        Model::find_by_pid_in_tenant(&boot.app_context.db, pid, "other-tenant")
            .await
            .is_err(),
        "User must not be found in another tenant"
    );
}
...
async fn can_find_by_pid() {
    ...
    let existing_user = Model::find_by_pid(
        &boot.app_context.db,
        uuid::Uuid::parse_str("11111111-1111-1111-1111-111111111111").unwrap(),
    )
    .await;
    let non_existing_user_results = Model::find_by_pid(
        &boot.app_context.db,
        uuid::Uuid::parse_str("23232323-2323-2323-2323-232323232323").unwrap(),
    )
    .await;
...
async fn magic_link() {
    ...
    let updated_user = Model::find_by_pid(
        &boot.app_context.db,
        uuid::Uuid::parse_str("11111111-1111-1111-1111-111111111111").unwrap(),
    )
    .await
