...
async fn can_register() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let saved_user =
            users::Model::find_by_email_in_tenant(&ctx.db, email, DEFAULT_TENANT_CODE).await;
...
#[tokio::test]
#[serial]
async fn profile_and_current_reject_pid_bound_to_other_tenant() {
    request::<App, _, _>(|request, ctx| async move {
        let other_user = users::Model::create_with_password(
            &ctx.db,
            &users::RegisterParams {
                ...
                manager_pid: None,
                position_ids: None,
                email: "other-tenant@example.com".to_string(),
        ...
        .await
        .expect("Failed to create cross-tenant user");

        let jwt_config = ctx.config.get_jwt_config().expect("JWT config");
        let mut claims = serde_json::Map::new();
        claims.insert(
            "tenant".to_string(),
            serde_json::Value::String(DEFAULT_TENANT_CODE.to_string()),
        );
        claims.insert(
            "iat".to_string(),
        ...
        let token = other_user
            .generate_jwt(&jwt_config.secret, jwt_config.expiration, claims)
        ...
        assert_eq!(
            profile.status_code(),
            401,
            "Profile must reject wrong-tenant PID"
        );
        assert_eq!(
            current.status_code(),
            401,
            "Current must reject wrong-tenant PID"
        );
    })
    .await;
}
...
#[rstest]
#[case("login_with_valid_password", "Test1234")]
...
async fn can_login_with_verify(#[case] test_name: &str, #[case] password: &str) {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let user = users::Model::find_by_email_in_tenant(&ctx.db, email, DEFAULT_TENANT_CODE)
            .await
            .unwrap();
...
#[tokio::test]
#[serial]
async fn login_with_revoked_cookie_still_succeeds() {
    configure_insta!();

    request::<App, _, _>(|request, ctx| async move {
        let login_data = prepare_data::init_user_login(&request, &ctx).await;
        loco::middleware::session_guard::revoke_user(&login_data.user.pid.to_string())
            .await
            .expect("Failed to revoke user session");
        let cookie_header = HeaderValue::from_str(&format!("loco_token={}", login_data.token))
        ...
        let response = request
            ...
            .add_header(HeaderName::from_static("cookie"), cookie_header)
            .await;
        assert_eq!(
            response.status_code(),
            200,
            "Login should succeed even with an old revoked session cookie: {}",
            response.text()
        );
    })
    .await;
...
async fn can_reset_password() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let user = users::Model::find_by_email_in_tenant(
            &ctx.db,
        ...
        let user = users::Model::find_by_email_in_tenant(&ctx.db, &user.email, DEFAULT_TENANT_CODE)
            .await
            .unwrap();
...
async fn can_auth_with_magic_link() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        std::env::set_var("MAGIC_LINK_ALLOWED_DOMAINS", "example.com");
